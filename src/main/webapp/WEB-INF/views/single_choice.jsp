...
<head>
    ...
    <script>
            ...
            console.log(coursename);
                ...
                $.ajax({
                    ...
                    success: function (words) {
                        var acc=[];
                        for (i in words) {
                            if(words[i].type==0&&words[i].name==coursename){
                                x="<tr><td>" +"<input type='checkbox'/>"+ "</td>"+"<td>"+words[i].id+"</td>"+"<td>" +words[i].title+ "</td>"+"<td>" +words[i].selectA+ "</td>"+"<td>" +words[i].selectB + "</td>"+"<td>" +words[i].selectC+ "</td>"+"<td>" +words[i].selectD+ "</td>"+"<td>" +words[i].anwser+ "</td></tr>";
                                $("#tb").append(x);
        ...
        function saveeditWord() {
            ...
            console.log(id+" "+title+" "+a+" "+b+" "+c+" "+d+" "+answer);
            $.ajax({
                url: "${pageContext.request.contextPath}/teacherController/updateAllTest",//获取数据的URL
                data:{"id":id,"title":title,"a":a,"b":b,"c":c,"d":d,"e":null,"answer":answer},//前端将数据给后端
                type: "POST",//HTTP请求方法
        ...
        function saveWord() {
            ...
            console.log(courseid+" "+title+" "+a+" "+b+" "+c+" "+d+" "+answer);
            $.ajax({
                url: "${pageContext.request.contextPath}/teacherController/insertTest",//获取数据的URL
                data:{"id":courseid,"title":title,"a":a,"b":b,"c":c,"d":d,"e":null,"answer":answer,"type":0},//前端将数据给后端
                type: "POST",//HTTP请求方法
                dataType : 'JSON',//获取数据执行方式
                success:function(data){
                    if(data===1){//传入为JSON对象格式
        ...
        function editWord(){//data-toggle="modal"  data-target="#myModal-1"    //模态框
            var vals = [];
            ...
            if(vals.length<1||vals.length>=2) {
                ...
                // })
                //var y=userArr[1];
                $("input[id='id-1']").val(w_Arr[0]);
                $("input[id='title-1']").val(w_Arr[1]);
                $("input[id='a-1']").val(w_Arr[2]);
                $("input[id='b-1']").val(w_Arr[3]);
                $("input[id='c-1']").val(w_Arr[4]);
                $("input[id='d-1']").val(w_Arr[5]);
                $("input[id='answer-1']").val(w_Arr[6]);
                //数据显示在模态框
            ...
            <div class="ibox float-e-margins">
                <div class="toolbar" style="height: 20px;">
                    ...
                    <ul class="nav nav-pills">
                        ...
                        <li role="presentation"class="active">
                            <button id="editButton" onclick="editWord()" type="button" data-toggle="modal"><i class="glyphicon glyphicon-edit"></i>
                                修改
                            </button>
                        </li>
                        <li role="presentation"class="active">
                            <button id="deleteButton" onclick="deleteWord()" type="button"  data-toggle="modal"><i class="glyphicon glyphicon-remove"></i>
                                删除
                            </button>
                        </li>
                ...
                <div class="ibox-content">
                    <table class="table table-striped table-bordered table-hover  dataTables-example" id="tb" >
                        <tr>
                            <th>选中</th><th>题号</th><th>题目</th><th>选项A</th><th>选项B</th><th>选项C</th><th>选项D</th><th>答案</th>
                        </tr>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h2 class="modal-title" id="myModalLabel">新增题目</h2>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="button1" class="btn btn-primary" onclick="saveWord()" >Save changes</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="myModal-1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    <button type="button" id="button1-1" class="btn btn-primary" onclick="saveeditWord()" >Save changes</button>
                </div>
            </div>
        </div>
    </div>
